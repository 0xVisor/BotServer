...
void GrowtopiaBot::onLoginRequested()
{
	...
	if (botmeta == "random") {
		packet = "requestedName|RaidedByArtemis\nf|1\nprotocol|146\ngame_version|" + ver + "\nfz|5367464\nlmode|0\ncbits|0\nplayer_age|18\nGDPR|1\nhash2|" + hash2 + "\nmeta|" + generateMeta() + "\nfhash|-716928004\nrid|" + generateRid() + "\nplatformID|0\ndeviceVersion|0\ncountry|us\nhash|" + hash + "\nmac|" + generateMac() + "\nwk|" + generateRid() + "\nzf|-496303939" + token;
	}
	else {
		string packet = "requestedName|RaidedByArtemis\nf|1\nprotocol|146\ngame_version|" + ver + "\nfz|5367464\nlmode|0\ncbits|0\nplayer_age|18\nGDPR|1\nhash2|" + hash2 + "\nmeta|" + botmeta + "\nfhash|-716928004\nrid|" + generateRid() + "\nplatformID|0\ndeviceVersion|0\ncountry|us\nhash|" + hash + "\nmac|" + generateMac() + "\nwk|" + generateRid() + "\nzf|-496303939" + token;
	}
	cout << packet;
	SendPacket(2, packet, peer);
	SendPacket(2, "action|enter_game\n", peer);
	SendPacket(2, "action|getDRAnimations", peer);
	currentWorld = "";
}
...
void GrowtopiaBot::userLoop() {
	if (timeFromWorldEnter > 200 && currentWorld != worldName) {
		if (worldName == "" || worldName == "-") {
			...
			SendPacket(2, "action|getDRAnimations", peer);
			SendPacket(2, "action|enter_game", peer);
